...
const ex_screen = document.querySelector('.ex-screen');
...
function AssignNumber(e) {
  ...
  if (evaluation.length >= 2) {
    ...
    ex_screen.textContent = first_number;
...
function AssignOperation(e) {
  ...
  ex_screen.textContent = `${first_number}${current_operator}`;
}
...
function Render(e) {
  ...
  last_operator_button
    ? last_operator_button.classList.remove('selected_operation')
    : null;
  new_operator_button
    ? new_operator_button.classList.add('selected_operation')
    : null;
